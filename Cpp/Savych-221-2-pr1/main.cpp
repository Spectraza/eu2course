...
Train **getTrains(int size) { //свторюємо функцію для отримання інформації створенних поїздів
    ...
    for (int i = 0;
         i < size; ++i) { //створюємо цикл для введення даних всіх поїздів, в залежності від заданної кількості поїздів
...
void printTrain(
        Train *train) { //ця функція створена для виведення інформації про поїзд, який може приїхати в заданний час, який вам треба
    time_t time = train->getDepartureTime(); //вивзмаємо функцію час з классом
    printf("Name: %s, number: %i, departureTime: %s", train->getName()->c_str(), train->getTrainNumber(),
           ctime(&time)); //виводимо інформацію
}

bool *trainsAfterDate(Train **trains,
                      int size) {// ця функція створена щоб дізнатися чи є інший поїзд щоб повернутися в інший день
...
int main() {
    nlohmann::json train = nlohmann::json::parse(R" {
        "name": "Kyiv",
        "number": 1
    }
    ");
